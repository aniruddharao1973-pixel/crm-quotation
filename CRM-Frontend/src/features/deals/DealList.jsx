...
const DealList = () => {
  ...
  const fetchAllDealsForExport = async () => {
    try {
      ...
      const res = await dispatch(
        fetchDeals({
          page: 1,
          limit: 100000, // 🚀 triggers full fetch
          sortBy: sortConfig.column,
          sortOrder: sortConfig.order,
          search: debouncedSearch || undefined,
        }),
      ...
      // ✅ restore original page immediately
      dispatch(
        fetchDeals({
          page,
          limit,
  ...
  const prepareExportData = (data) => {
    return data.map((deal) => ({
      ...
      // ✅ EXTRA DETAILS (important for export)
      Amount: deal.amount || "",
                    ...
                    <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden py-1">
                      ...
                      <button
                        onClick={() => {
                          ...
                          saveAs(blob, `deals_page_export_${Date.now()}.csv`);
                          setShowExportDropdown(false);
                        }}
                        className="block w-full text-left px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-[#3B2E7E]/5 hover:text-[#3B2E7E]"
                      >
                ...
                {sortedDeals.map((deal, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold ${stageColor.bg} ${stageColor.text}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full mr-1.5 ${stageColor.dot}`}
                          />
